...
async function communitySignals(
  ...
  const accuracy = await pool.query<{ rating: string; captured_at: Date }>(
    `select rating, captured_at
       from condition_accuracy_rating
      where beach_id = $1 and captured_at > now() - interval '7 days'`,
    [beachId],
  );
  ...
  return {
    ...
    accuracyRatings: accuracy.rows.map((row) => ({
      ...
      accurate: Number(row.rating) >= 0.5,
    })),
  };
}
